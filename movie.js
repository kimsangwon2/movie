...
const options = {
  ...
  headers: {
    ...
    Authorization: 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI4OGVlZTk5NzUwY2NkOWExOTA1ZDRlMWNiN2RmM2Q4MyIsInN1YiI6IjY1OGVhOWM1MWY3NDhiNmRlMzk4M2QyZSIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.Dl1ghE54NcS5IvfmNvMbQr69R26-YzNJ8fY8xFwazyQ'
  }
};
  ...
  .then(response => {
    ...
    let moviecard = movie => {
      ...
      let searchresult = () => {
        ...
        let searchmoive = movie.filter(search => search.title.includes(searchinputcase))
        movie(searchmoive);
      }
    ...
    arr.forEach(movie => {
      ...
      movie_card.addEventListener('click',() => {  //movie_card가 movie 하나의 객체를 다 포함하기 때문에 movie_card에 이벤트 발생
